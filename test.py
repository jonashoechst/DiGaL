...
class Player:
    def __str__(self): return ', '.join(map(str, reversed(self.__dict__.values())))
...
class Game:
    # Static methods
    def status(self): return 'Status['+', '.join(reversed(self.players[0].__dict__.keys())).lower()+']: '+' - '.join(map(str, self.players))
